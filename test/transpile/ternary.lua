...
local transpile_ok, expr_code, run_js, run_lua_source =
  H.transpile_ok, H.expr_code, H.run_js, H.run_lua_source
...
test("ternary integration: falsy consequent is not or'd away", function()
  local output = run_lua_source(
    "local x = (function() if true then return false else return 0 end end)()\nprint(tostring(x))\n"
  )
