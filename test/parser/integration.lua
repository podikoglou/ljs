...
test("parse: == produces BinaryExpression with operator ==", function()
  local ast, err = parser.parse("1 == 2")
  assert(ast ~= nil, "expected successful parse: " .. tostring(err))
  assert_eq(ast.body[1].expression.operator, "==")
  assert_eq(ast.body[1].expression.left.value, 1)
  assert_eq(ast.body[1].expression.right.value, 2)
end)
