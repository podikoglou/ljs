...
test("preamble contains all helpers", function()
  ...
  assert(p:find("local function _ljs_to_number"), "expected _ljs_to_number")
  assert(p:find("local function _ljs_to_boolean"), "expected _ljs_to_boolean")
...
test("load function returns JS function callable with args", function()
  local fn, err = ljs.load("function add(a, b) { return a + b; }\nadd")
  assert_eq(err, nil)
  assert(fn ~= nil, "expected function")
  local add = fn()
...
test("load function returns JS arrow callable with args", function()
  local fn, err = ljs.load("let mul = (a, b) => a * b; mul")
  assert_eq(err, nil)
  assert(fn ~= nil, "expected function")
  local mul = fn()
  assert(mul ~= nil, "expected inner function")
  assert_eq(mul(3, 4), 12)
end)

test("run returns JS function callable with args", function()
  local add, err = ljs.run("function add(a, b) { return a + b; }\nadd")
  assert_eq(err, nil)
  assert(add ~= nil, "expected function")
  assert_eq(add(3, 4), 7)
end)
